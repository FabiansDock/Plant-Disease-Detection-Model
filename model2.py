...
data_path = Path('data/')
image_path = data_path / "cassava_mango_tomato"
...
class TinyVGG(nn.Module):
    ...
    def forward(self, x: torch.TensorType):
        x = self.conv_block_6(self.conv_block_5(self.conv_block_4(
            self.conv_block_3(self.conv_block_2(self.conv_block_1(x))))))
        # print(x.shape)
...
train_results = train(model_0, train_custom_dataloader,
                      test_custom_dataloader, loss_fn, optimizer, epochs=50)

MODELS_PATH = Path("models/")
MODELS_PATH.mkdir(parents=True, exist_ok=True)

MODELS_NAME = "disease_category.pth"
